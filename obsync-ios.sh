...
function fail_or_create_mount_path_not_exists() {
    if [[ ! -d "${_cmd_mount_path}" ]]; then
        if [ $_cmd_create -eq 1 ]; then
            mkdir -p "${_cmd_mount_path}"
        else
            fatal "Obsidian mount failed: ${_cmd_mount_path} does not exist."
        fi
    fi
}
...
function mount() {
    if [ "${#_cmd_mount_path}" -eq 0 ]; then
        error "mpath command option is missing '${_cmd_path}'"
    elif [ $_cmd_create -eq 0 ]; then
        fail_or_create_mount_path_not_exists
    fi

    mount -t ios null "${_cmd_mount_path}"
}
...
function usage() {
    if [[ $# -eq 0 ]]; then
        ...
        echo "  mount                 Initialize an obsidian repository."
        echo "    -c | --create         (optional) Create the mount directory if it doesn't exist."
        echo "    --mpath PATH          (required) Path for Obsidian app filesystem mount."
